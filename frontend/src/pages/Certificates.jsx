...
export default function Certificates() {
    ...
    return (
      <div className="grid min-h-[70vh] place-items-center p-4">
        <Card className="w-full max-w-xl p-6">
          <div className="text-lg font-extrabold tracking-tight">{t('certificates.title')}</div>
          <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">
            {t('certificates.blockedMsg')}
          </p>
          <div className="mt-4 flex flex-wrap gap-2">
            <Button variant="primary" onClick={() => (window.location.href = '/login')}>
              {t('certificates.goStudentLogin')}
            </Button>
  ...
  if (loading) {
    return (
      <Card className="p-5">
        <div className="text-sm text-slate-600 dark:text-slate-300">{t('certificates.loading')}</div>
      </Card>
    );
  }
  ...
  if (error) {
    return (
      <Card className="p-5">
        <div className="text-sm font-semibold text-slate-900 dark:text-white">{t('certificates.couldNotLoad')}</div>
  ...
  return (
    <Card className="p-5">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <div className="text-xl font-extrabold tracking-tight text-slate-900 dark:text-white">{t('certificates.title')}</div>
      ...
      <div className="mt-4 overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-800">
        <table className="w-full border-collapse">
          <thead className="bg-slate-50 text-left text-xs font-semibold uppercase tracking-wide text-slate-600 dark:bg-slate-950/40 dark:text-slate-300">
            <tr>
              <th className="px-4 py-3">{t('certificates.certificateId')}</th>
              <th className="px-4 py-3">{t('certificates.course')}</th>
              <th className="px-4 py-3">{t('certificates.score')}</th>
              <th className="px-4 py-3">{t('certificates.issued')}</th>
              <th className="px-4 py-3 text-right">{t('certificates.download')}</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
            {items.map((c) => (
              <tr key={c._id || c.certificateId} className="text-sm">
                ...
                <td className="px-4 py-3 text-slate-800 dark:text-slate-100">{c.course?.title || t('certificates.course')}</td>
                <td className="px-4 py-3 text-slate-700 dark:text-slate-200">
                  {typeof c.scorePercent === 'number'
                    ? `${c.scorePercent}%${typeof c.passPercent === 'number' ? ` (${t('certificates.pass')} ${c.passPercent}%)` : ''}`
                    : '-'}
                </td>
                <td className="px-4 py-3 text-slate-700 dark:text-slate-200">
