  ...
  return (
    <div className="grid min-h-[70vh] place-items-center p-4">
      <Card className="w-full max-w-xl p-6">
        <div className="text-lg font-extrabold tracking-tight">{t('profile.title')}</div>
        <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">
          {t('profile.blockedMsg')}
        </p>
...
export default function Profile() {
  ...
  async function linkGoogle(credential) {
    try {
      setGoogleMsg(t('profile.linkingGoogle'));
      ...
      setUser(data.user);
      setGoogleMsg(t('profile.googleLinked'));
        ...
        try {
          google.accounts.id.initialize({
            ...
            callback: (resp) => {
              ...
              if (!cred) {
                setGoogleMsg(t('profile.googleMissingCredential'));
                return;
              }
              linkGoogle(cred);
            }
  ...
  if (loading) {
    return (
      <Card className="p-5">
        <div className="text-sm text-slate-600 dark:text-slate-300">{t('profile.loading')}</div>
      </Card>
    );
  }
  ...
  if (error) {
    return (
      <Card className="p-5">
        <div className="text-sm font-semibold text-slate-900 dark:text-white">{t('profile.couldNotLoad')}</div>
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-2xl font-extrabold tracking-tight text-slate-900 dark:text-white">{t('profile.title')}</div>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <Field label={t('profile.headline')}>
              <Input
                ...
                placeholder={t('profile.headlinePh')}
              />
            </Field>
            <Field label={t('profile.location')}>
              <Input value={form.location} onChange={(e) => setForm((v) => ({ ...v, location: e.target.value }))} placeholder={t('profile.locationPh')} />
            </Field>
      ...
      <Card className="p-5">
        <div className="text-sm font-extrabold">{t('profile.securityTitle')}</div>
        <div className="mt-1 text-sm text-slate-600 dark:text-slate-300">{t('profile.securitySubtitle')}</div>

        {!isGoogleLinked && googleClientId && (
          <div className="mt-4">
            <div className="text-sm text-slate-600 dark:text-slate-300">{t('profile.linkGoogle')}</div>
