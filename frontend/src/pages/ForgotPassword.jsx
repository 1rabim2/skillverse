  ...
  return (
    <div className="grid min-h-[70vh] place-items-center p-4">
      <Card className="w-full max-w-md p-6">
        <div className="text-xl font-extrabold tracking-tight text-slate-900 dark:text-white">{t('passwordReset.forgotTitle')}</div>
        ...
        <form onSubmit={submit} className="mt-5 space-y-4">
          <div className="space-y-2">
            <div className="text-xs font-semibold text-slate-600 dark:text-slate-300">{t('passwordReset.email')}</div>
            <Input value={email} onChange={(e) => setEmail(e.target.value)} type="email" required placeholder={t('passwordReset.emailPh')} />
          </div>
          <Button type="submit" variant="primary" disabled={busy}>
            {busy ? t('passwordReset.sending') : t('passwordReset.sendReset')}
          </Button>
        </form>
